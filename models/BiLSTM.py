...
class BiLSTM(nn.Module):
    ...
    def forward(self, context_idxs, pos, context_ner, context_char_idxs, context_lens, h_mapping, t_mapping,
        ...
        batch_size, doc_len, bi_hidden_size = context_output.size()
        context_output = context_output.view(batch_size, doc_len*2,-1)
        ...
        #pos_context_output = context_output[:,:,0,:]
        #neg_context_output = context_output[:,:,1,:]
        ...
        #print(cumsum_idx[0])
        #print(sent_lengths[0])
        #print(context_idxs[0], context_starts[0])
        #assert(False)
        ...
        s_t_sent_rep = torch.cat([start_re_output, end_re_output, sent_embeds],-1)
        sent_pred = self.linear_sp(s_t_sent_rep).squeeze(-1)
        #print(sent_pred.size())

        return predict_re, sent_pred
